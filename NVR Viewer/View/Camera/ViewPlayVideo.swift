...
// =======================================================
// MARK: - ViewPlayVideo (MP4 Download + Local Playback)
// =======================================================
import SwiftUI
import AVKit
import AVFoundation
        ...
        VStack(spacing: 0) {
            ...
            if let player = player {
                ...
                if isReadyToPlay {
                    ...
                    VStack(spacing: 0) {      // 👈 spacing 0 so pill touches the video
                        ...
                        VideoPlayer(player: player)
                            ...
                            .clipped()
                            .onDisappear {
                                print("[DEBUG] ⏸ View disappeared, pausing & cleaning up")
                                player.pause()
                                cleanupTempFolder()
                            }

                        if isMP4InvalidURL(urlMp4String) {
                            DownloadView(urlString: urlMp4String, fileName: frameTime)
                                .frame(maxWidth: .infinity, alignment: .leading)
                                .padding(.horizontal, 10)
                                .padding(.vertical, 10)
                                .background(
                                    BottomRoundedRectangle(radius: 22)   // 👈 only bottom corners rounded
                                        .fill(
                                            LinearGradient(
                                                colors: [
                                                    cBlue.opacity(0.6),                 // your main blue
                                                    cBlue.opacity(0.95)
                                                ],
                        ...
                        // MARK: Save Clip pill card spacer / layout fix
                        HStack { }
                ...
                VStack(spacing: 8) {
                    ...
                    .frame(maxWidth: .infinity)
                    .frame(height: 290)   // keep same height as video player
                    .padding(.horizontal, 16)
                }
                .frame(maxWidth: .infinity)
