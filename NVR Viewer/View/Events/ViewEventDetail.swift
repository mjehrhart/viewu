...
func downloadImage(from urlString: String) async -> UIImage? {
    guard let url = URL(string: urlString) else {
        Log.warning(page: "ViewEventDetail",
                           fn: "downloadImage", "Invalid URL - \(urlString)")
        return nil
    }
    ...
    do {
        ...
        Log.error(page: "ViewEventDetail",
                           fn: "downloadImage", "Error downloading image: \(error.localizedDescription)")
        return nil
    }
}
...
class ImageSaver: NSObject {
    ...
    @objc private func saveCompleted(_ image: UIImage,
        ...
        if let error = error { 
            Log.error(page: "ViewEventDetail",
                               fn: "ImageSaver", "Save Error \(error.localizedDescription)")
        }
    }
}
...
func isValidMp4URL(_ option: String?) -> Bool {
    ...
    guard let value = option?.trimmingCharacters(in: .whitespacesAndNewlines),
          ...
          Log.error(page: "ViewEventDetail",
                           fn: "isValidMp4URL", "Invalid MPR URL: \(option ?? "")")
        return false
    }
    return true
}
